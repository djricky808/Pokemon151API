...
const setActive = (elm) => {
  console.log(elm);
  if (elm.innerHTML === 'Main Collection') {
    console.log(faveContainer);
    faveContainer.classList.remove('active');
    mainContainer.classList.add('active');
  } else {
    console.log(mainContainer);
    console.log(faveContainer);
    mainContainer.classList.remove('active');
    faveContainer.classList.add('active');
  }
}
...
const declareArrs = () => {
  ...
  containerArr = [mainPokemon, favePokemon];
  itemArr = [mainArr, faveArr];
}
...
const sortByPokedexNumber = (direction) => {
  ...
  for (i = 0; i < itemArr.length; i++) {
    ...
    itemArr[i].forEach((item) => {
      containerArr[i].append(item);
    });
  }
}
...
const sortPokemon = document.getElementById('sortPokemonBy');

sortPokemon.addEventListener('click', function () {
...
const updateFavStatus = (id, direction) => {
  ...
  const params = direction === 'toFaves' ? 
  [containerArr, starArr, classArr, addId(id)] : [containerArr.reverse(), starArr.reverse(), classArr.reverse(), deleteId(id)];
